...
(function() {
    ...
    var relPath = path.replace("/genias",""); 
    
    
    if (includeCSS) {
        document.write('<link rel="stylesheet" type="text/css" href="' + relPath + '/jscript/ext/resources/css/ext-all' + suffix + '-debug.css"/>');
    }
    
    document.write('<script type="text/javascript" src="' + relPath + '/jscript/ext/ext-all' + (rtl ? '-rtl' : '') + '.js"></script>');
    ...
    if (neptune) {
        ...
        neptunePath = (repoDevMode ? relPath + '/..' : relPath) +            
            '/jscript/ext/packages/ext-theme-neptune/build/ext-theme-neptune' +
            (repoDevMode ? '-dev' : '') + '.js';
