...
dhtmlXGridObject.prototype.forceFullLoading=function(b){for(var b=b||50,a=0;a<this.rowsBuffer.length;a++)if(!this.rowsBuffer[a]){if(this.callEvent("onDynXLS",[a,b])){var d=this;this.load(this.xmlFileUrl+getUrlSymbol(this.xmlFileUrl)+"posStart="+a+"&count="+b,function(){window.setTimeout(function(){d.forceFullLoading(b)},100)},this._data_type)}break}};dhtmlXGridObject.prototype.setAwaitedRowHeight=function(b){this._srdh=parseInt(b)};
...
dhtmlXGridObject.prototype._update_srnd_view=function(){var b=Math.floor(this.objBox.scrollTop/this._srdh),a=b+this._get_view_size();if(this.multiLine){for(var d=this.objBox.scrollTop,b=0;d>0;)d-=this.rowsCol[b]?this.rowsCol[b].offsetHeight:this._srdh,b++;a=b+this._get_view_size();b>0&&b--}a+=this._srnd_pr||0;if(a>this.rowsBuffer.length)a=this.rowsBuffer.length;for(var c=b;c<a;c++)if(!this.rowsCol[c]){var e=this._add_from_buffer(c);if(e==-1){if(this.xmlFileUrl)this._current_load=[c,this._dpref?this._dpref:
a-c],this.callEvent("onDynXLS",[c,this._current_load[1]])&&this.load(this.xmlFileUrl+getUrlSymbol(this.xmlFileUrl)+"posStart="+c+"&count="+this._current_load[1],this._data_type);return}else if(this._tgle&&(this._updateLine(this._h2.get[this.rowsBuffer[c].idd],this.rowsBuffer[c]),this._updateParentLine(this._h2.get[this.rowsBuffer[c].idd],this.rowsBuffer[c])),c&&c==(this._realfake?this._fake:this)._r_select)this.selectCell(c,this.cell?this.cell._cellIndex:0,!0)}if(this._fake&&!this._realfake&&this.multiLine)this._fake.objBox.scrollTop=
