        ...
        if(pgridCache[id]){
            ...
            Ext.Ajax.request({
                ...
                success: function(response, options){
                    ...
                    //---update pgrid 
                    propsGrid.id=form.id;
                    delete form.id;
                    //----set source with returned data
                    propsGrid.setSource(form);
                    //----update gridview
                    //----update Grid whith data
                    //---convert source data to propper Form model 
                    mygrid.store.data.items[mygrid.store.data.keys.indexOf(id)]=Ext.ModelManager.create(form,'FormsModel');
                    mygrid.getView().refresh(true);
                    //---update cache
                    pgridCache[id]=propsGrid.getSource();
                    pgridCache[form.idform]=propsGrid.getSource();
                    propsGrid.setLoading(false);
                },
                // NO errors ! ;)
                failure: function(response,options){
