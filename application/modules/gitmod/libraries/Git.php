...
class GitRepo {
	...
	/**
	 * Runs a 'git ls-files -o --exclude-standard' call
	 *
	 * Accept a convert to HTML bool
	 * 
	 * @access public
	...
	public function staged() {
		$msg = $this->run("diff --name-only --cached");
	 ...
	 * @return  string
	 */
	public function commit_file($file,$message = "", $commit_all = true) {
	 ...
	 * @return  string
	 */
	public function commit($message = "", $commit_all = true,$author=null) {
		$flags = $commit_all ? '-av' : '-v';
		$flags.= $author ? ' --author="'.$author.'"':'';
		return $this->run("commit ".$flags." -m ".escapeshellarg($message));
	}
