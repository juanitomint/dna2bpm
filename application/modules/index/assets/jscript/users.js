$(document).ready(function(){
    ...
    $('.delete_user_db').live('click',function(){
        iduser=$(this).val();
        url=globals.base_url+'user/admin/delete_user_db/'+iduser;
        mydialog=$('<div title="Confirm"><h1>You are about to delete user:'+iduser+' from DB</div>').dialog({
            ...
            buttons:{
                ...
                'Yes, do it':function(){
                    $(this).dialog('close');
                    $('#center').html('<img src="'+globals.base_url+'css/ajax/loader18.gif"/>');
                    $.ajax({
    ...
    $('#pForm .btn_save').live('click',function(){
        ...
        url=globals.base_url+'user/admin/save_user/'+iduser;
...
function loadUsers(idgroup,order,active,query,page){
    ...
    url=globals.base_url+'user/admin/get_users/'+idgroup+'/'+order+'/'+page;
    //---load dialog
            ...
            $("#accordion").accordion({
                change: function(event, ui) {
                    ...
                    if($('#pForm form input[name=id]').val()!=id){
                        url=globals.base_url+'user/admin/get_properties/'+id+'/'+idgroup;
                        $('#pForm').html('<img src="'+globals.base_url+'css/ajax/loader18.gif"/>Loading properties for:'+id);
...
function loadProperties(id,idgroup){
    url=globals.base_url+'user/admin/get_user_properties/'+id+'/'+idgroup;
    $('#pForm').html('<img src="'+globals.base_url+'css/ajax/loader18.gif"/>Loading properties for:'+id).load(url,'',
        function(){
            $('#eMSG').html('');
            initButtons();
            //---init date picker
            ...
            $('#btn_editPerm').click(function(){
                ...
                $("#permDialog" ).load(globals.base_url+'user/admin/get_apps/'+idu,postData,function(){
                    $("#permDialog" ).dialog({
                        modal: true,
                        resizable: true,
                        height: 400,
            ...
            //----4 TEST
            $('#test').click(function(){
                window.location=globals.base_url+'user/authenticate/byhash/'+$(this).attr('nick')+'/'+$(this).attr('passw');
            });
        });
}
